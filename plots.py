...
def save_plots(parallel_coordinates_data,
    ...
    for j in range(len(mass_plots)):
        ...
        mass_plot_path = os.path.join(save_dir, f"mass_plot_{experiment_name}_{j}_{N_TRIALS}.png")
        connectivity_plot_path = os.path.join(save_dir, f"connectivity_plot_{experiment_name}_{j}_{N_TRIALS}.png")
        symmetry_plot_path = os.path.join(save_dir, f"symmetry_plot_{experiment_name}_{j}_{N_TRIALS}.png")
        beam_cont_plot_path = os.path.join(save_dir, f"beam_cont_plot_{experiment_name}_{j}_{N_TRIALS}.png")
    ...
    for n_idx in range(len(weights)):
        figure, axs = plt.subplots(nrows=len(unique_truss_types), ncols=1, figsize=(16, 20))  # Adjust height as needed
